...
function hotel_galaxy_custom_breadcrums($id_get){
 ...
 elseif($id_get=='category'){
     ...
     $title=__('<a href="'. home_url() .'">' . "Trang chủ /" . '</a>', 'hotel-galaxy');
 }elseif($id_get=='404'){
     $id=__('Lỗi 404','hotel-galaxy');
     $title=__('Không tìm thấy kết quả tìm kiếm !!!','hotel-galaxy');    
 }
 
 elseif($id_get=='search'){
     $id=get_search_query();
     $title=__('Kết quả tìm kiếm cho:','hotel-galaxy');
     $search_error=__('Không tìm thấy kết quả tìm kiếm !!!','hotel-galaxy');
 }
 ?>
